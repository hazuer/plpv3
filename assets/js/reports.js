$(document).ready(function () {
	...
	let table = $('#tbl-reports').DataTable({
		...
		"columns": [
			...
			{ title: `CP`, name: `cp`, data: `cp` },          //23
			{ title: `id_location`, name: `id_location`, data: `id_location` }          //24
		],
		'order': [[14, 'desc']]
	});
	...
	$(`#tbl-reports tbody`).on(`click`, `#id-logger`, function () {
		...
		if (row.status_desc === 'Nuevo' || row.status_desc === 'Devuelto') {
			$('#container-change-location').show();

			$('#id_package_db').val(row.id_package);
			$('#select-location').data('currentLocationDesc', row.location_desc);
			$('#select-location').val('');
			$('#select-location option').show();
			$('#select-location option').filter(function() {
				return $(this).text().trim() === row.location_desc;
			}).hide();
		}
	});
    ...
    let currentLocationDesc = $(this).data('currentLocationDesc');
    let newLocationDesc = $(this).find('option:selected').text();
    $('#txt_mv_location').val(`Paquete actualizado: de ${currentLocationDesc} a ${newLocationDesc}`);
